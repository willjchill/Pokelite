...
static void parsePokemonJSON(const std::string& filename) {
    ...
    for (const auto& pokemon : root) {
        ...
        // Format: game_logic/sprites/{id:03d}_{name} (e.g., "game_logic/sprites/001_bulbasaur")
        std::string spriteDir;
        if (pokemon.isMember("sprite_dir") && pokemon["sprite_dir"].isString()) {
            spriteDir = pokemon["sprite_dir"].asString();
            // If it doesn't start with "game_logic/", prepend it
            if (spriteDir.find("game_logic/") == std::string::npos) {
                spriteDir = "game_logic/" + spriteDir;
            }
        } else {
            // Construct sprite directory from ID and name
            // Format: game_logic/sprites/{id:03d}_{name}
            char dirName[128];
            snprintf(dirName, sizeof(dirName), "game_logic/sprites/%03d_%s", id, name.c_str());
            spriteDir = dirName;
        }
...
bool initializePokemonDataFromJSON() {
    ...
    std::string movesPath = "firered_moves.json";
    std::ifstream test1(movesPath);
    if (!test1.is_open()) {
        movesPath = "src/game_logic/firered_moves.json";
    ...
    // Parse Pokemon data
    std::string pokedexPath = "firered_full_pokedex.json";
    std::ifstream test3(pokedexPath);
    if (!test3.is_open()) {
        pokedexPath = "src/game_logic/firered_full_pokedex.json";
...
PokemonSpeciesData getPokemonSpeciesData(int dexNumber) {
    ...
    return {dexNumber, "Unknown", Type::NORMAL, Type::NONE, 50, 50, 50, 50, 50, 50, "game_logic/sprites/006_charizard"};
}
